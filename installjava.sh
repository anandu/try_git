...
##
# $Id: installjava.sh 182 2011-10-06 03:39:11Z glenn $
...
function version {
  echo "
  $0 ($Id: installjava.sh 182 2011-10-06 03:39:11Z glenn $)
  Copyright Rimuhosting.com
  Install distribution independant copy of Sun Java JDK
"
}
...
#
function usage {
  echo " Usage: $0 [--jre (5 or 6)] [--64bit] [--32bit] [--noprompt]

  Option:         Description:
  --64bit         force 64bit install (default matches os)
  --32bit         force 64bit install (default matches os)
  --jre           choose the java version (default is 6, also accept v5 and v7)
...
function parsecommandline {
  while [ -n "$1" ]; do
    ...
    --jre)
      shift
      if [[ "$1" != "5" && "$1" != "6" && "$1" != "7" ]]; then
        ERRORMSG="$PARAM must be followed by the desired jdk version (defaults to 6)"
        return 1
      fi
      JRE="$1"
      ;;
...
function installjava {
  ...
  if [[ "$JRE" == "7" ]]; then
    echo "! Java 7 is pretty new, and it is a bit different from v6. Please keep that in mind when developing."
  fi
